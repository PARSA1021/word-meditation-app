        ...
        <div className="max-w-3xl mx-auto space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <Link
                href="/"
                className="w-10 h-10 flex items-center justify-center rounded-sm bg-slate-50 text-brand-deep hover:bg-brand-primary hover:text-white transition-all active:scale-90 border border-brand-primary/5"
              >
            ...
            {/* Premium Search Mode Toggle */}
            <div className="relative flex p-1 bg-slate-50 rounded-sm border border-brand-primary/5 scale-90 origin-right md:scale-100 shadow-sm">
              <div
                className="absolute top-1 bottom-1 w-[calc(50%-4px)] bg-white rounded-sm shadow-sm ring-1 ring-brand-primary/5 transition-all duration-500 cubic-bezier(0.4, 0, 0.2, 1)"
              ...
              {(["text", "source"] as const).map((m) => (
                <button
                  key={m}
                  onClick={() => setMode(m)}
                  className={`relative z-10 px-5 py-2 rounded-sm text-[10px] font-black transition-colors duration-300 uppercase tracking-[0.2em]
                        ${mode === m ? "text-brand-primary" : "text-slate-400"}`}
                >
                  {m === "text" ? "Text" : "Source"}
                </button>
              ))}
