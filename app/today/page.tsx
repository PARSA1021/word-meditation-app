...
export default function TodayPage() {
  ...
  return (
    <div
      className={`min-h-[100dvh] pb-20 relative overflow-x-hidden transition-colors duration-500 bg-[#F2F2F7] ${
        textSize === "large" ? "text-size-large" : ""
      }`}
    >
      {/* Header */}
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-slate-100 px-4 sm:px-6 py-4 sm:py-5">
        <div className="max-w-xl md:max-w-2xl lg:max-w-3xl mx-auto">
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex items-center gap-2.5 sm:gap-3.5 min-w-0">
              <Link
                href="/"
                className="w-9 h-9 sm:w-10 sm:h-10 shrink-0 flex items-center justify-center rounded-2xl bg-slate-100 hover:bg-slate-200 active:scale-95 transition-all duration-200"
              >
              ...
              <div className="min-w-0">
                ...
                <p className="text-[9px] sm:text-[10px] font-bold uppercase tracking-[0.4em] sm:tracking-[0.5em] text-[#0099FF] mt-0.5">
                  DAILY VERSE
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-1.5 shrink-0">
              {/* 알림 구독 버튼 — 모든 화면에서 항상 표시 */}
              <PushSubscriber />

              {/* 글자 크기 토글 */}
              <button
                onClick={() => setTextSize((s) => (s === "normal" ? "large" : "normal"))}
                className={`w-9 h-9 sm:w-10 sm:h-10 flex items-center justify-center rounded-xl font-bold text-sm transition-all ${
                  textSize === "large"
              ...
              >
                <svg
                  className={`w-4 h-4 transition-transform ${isValidating ? "animate-spin" : ""}`}
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
      ...
      {/* Main Content */}
      <main className="w-full max-w-xl md:max-w-2xl lg:max-w-3xl mx-auto px-4 sm:px-6 pt-8 sm:pt-10 pb-16 flex flex-col items-center">
        ...
        <div className="mt-10 sm:mt-14 mb-8 text-center opacity-80">
      ...
      <nav className="fixed bottom-0 left-0 right-0 h-16 bg-white/90 backdrop-blur-xl border-t border-slate-200 md:hidden flex justify-between items-center px-6 z-40 transition-colors duration-500">
        ...
        <button
          ...
          className={`p-3 rounded-full shadow-sm transition-all ${
            isValidating
              ? "bg-slate-100 text-slate-400"
              : "bg-[#0099FF] text-white active:scale-95"
          }`}
        >
          <svg
      ...
      {/* PC 로딩 오버레이 — border-3 버그 수정 (border-[3px]로 수정) */}
      {isValidating && word && (
        <div className="fixed inset-0 bg-white/40 backdrop-blur-sm z-30 hidden md:flex items-center justify-center pointer-events-none transition-colors duration-500">
          <div className="flex flex-col items-center bg-white p-4 rounded-2xl shadow-sm border border-slate-100">
            <div className="w-6 h-6 border-[3px] border-[#0099FF] border-t-transparent rounded-full animate-spin" />
          </div>
        </div>
      )}
    </div>
