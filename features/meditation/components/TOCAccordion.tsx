...
const TOCAccordion = memo(function TOCAccordion({ 
 ...
 :"text-text-secondary hover:bg-slate-100/50 hover:text-brand-deep"
}
 `}
 ...
 {hasChildren && (
  ...
  <span className="text-[10px] font-black text-slate-300 group-hover:text-brand-primary/50 transition-colors">{node.wordCount}</span>
  <div className={`p-1 rounded-full transition-all duration-300 ${isOpen ?"bg-brand-primary/10 text-brand-primary rotate-180" :"bg-slate-100 text-slate-400"}`}>
  <svg
